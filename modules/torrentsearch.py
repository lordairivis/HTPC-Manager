...
regex_source = re.compile(r'(HDTV|HD-TV|HD\.TV|WEB-DL|WEB_DL|WEB\.DL|WEB_RIP|WEB-RIP|WEBRip|WEB\.RIP|BRRIP|BDRIP|BluRay(.*)REMUX)|BluRay(.*)\.(AVC|VC-1)\.|BluRay', re.I)
        ...
        try:
            if result:
                if 'torrents' in result:
                    for k, v in list(result['torrents'].items()):
